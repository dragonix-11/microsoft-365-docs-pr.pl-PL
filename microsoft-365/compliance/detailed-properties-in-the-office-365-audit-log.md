...
> [!TIP]
> Funkcja przekształcania JSON w programie Power Query w programie Excel umożliwia podzielenie kolumny **AuditData** na wiele kolumn, dzięki czemu każda właściwość ma własną kolumnę. Umożliwia to sortowanie i filtrowanie co najmniej jednej z tych właściwości. Aby dowiedzieć się, jak to zrobić, zobacz [Eksportowanie, konfigurowanie i wyświetlanie rekordów dziennika inspekcji](export-view-audit-log-records.md). 
...
|AddOnName|Nazwa dodatku, który został dodany, usunięty lub zaktualizowany w zespole. Typ dodatków w usłudze Microsoft Teams to bot, łącznik lub karta.|Microsoft Teams|
...
|AzureActiveDirectoryEventType|Typ zdarzenia usługi Azure Active Directory. Następujące wartości wskazują typ zdarzenia.  <br/> **0** — wskazuje zdarzenie logowania do konta.<br/> **1** — wskazuje zdarzenie zabezpieczeń aplikacji platformy Azure.|Azure Active Directory|
|ChannelGuid|Identyfikator kanału usługi Microsoft Teams. Zespół, w których znajduje się kanał, jest identyfikowany przez właściwości **TeamName** i **TeamGuid** .|Microsoft Teams|
|Channelname|Nazwa kanału usługi Microsoft Teams. Zespół, w których znajduje się kanał, jest identyfikowany przez właściwości **TeamName** i **TeamGuid** .|Microsoft Teams|
...
|EventSource|Określa, że zdarzenie miało miejsce w programie SharePoint. Możliwe wartości to **SharePoint** i **ObjectModel**.|SharePoint|
|ExternalAccess|W przypadku działania administratora programu Exchange określa, czy polecenie cmdlet było uruchamiane przez użytkownika w organizacji, przez personel centrum danych firmy Microsoft, konto usługi centrum danych, czy przez administratora delegowanego. Wartość **False** wskazuje, że polecenie cmdlet zostało uruchomione przez kogoś w organizacji. Wartość **True** wskazuje, że polecenie cmdlet zostało uruchomione przez personel centrum danych, konto usługi centrum danych lub administratora delegowanego.  <br/> W przypadku działania skrzynki pocztowej programu Exchange określa, czy do skrzynki pocztowej uzyskiwał dostęp użytkownik spoza organizacji.|Exchange|
|Extendedproperties|Właściwości rozszerzone zdarzenia usługi Azure Active Directory.|Azure Active Directory|
...
|Skrzynka pocztowaGuid|Identyfikator GUID programu Exchange skrzynki pocztowej, do których uzyskano dostęp.|Exchange (działanie skrzynki pocztowej)|
