...
- **Oceny ryzyka: Obejmują** one zwiększenie wyniku ryzyka dla nietypowych działań lub naruszeń zasad w przeszłości. Włączenie oceny ryzyka zwiększa wyniki ryzyka i prawdopodobieństwo wystąpienia alertów dotyczących tego typu działań. W przypadku nietypowych aktywności wyniki są zwiększane, jeśli wykryte działanie odbiega od typowego zachowania użytkownika. Na przykład znacząco wzrasta dzienny wzrost liczby pobieranych plików. Nietypowe działanie jest prezentowane jako zwiększenie wartości procentowej (na przykład "100% powyżej zwykłego działania") i będzie miało wpływ na ocenę ryzyka w inny sposób w zależności od aktywności. W przypadku użytkowników, którzy nawiązyli poprzednie naruszenia zasad, wyniki są zwiększane, jeśli użytkownik miał więcej niż jedną sprawę rozpoznaną wcześniej jako potwierdzone naruszenie zasad. Ocenę ryzyka można wybrać tylko w przypadku, gdy jest wybrany co najmniej jeden wskaźnik.
...
3. Zainstaluj przeglądarkę Microsoft Chromium Edge na urządzeniu końcowym, aby monitorować działania związane z przekazywaniem w chmurze. Zobacz [Pobieranie nowej aplikacji Microsoft Edge na podstawie Chromium](https://support.microsoft.com/help/4501095/download-the-new-microsoft-edge-based-on-chromium).
...
Jeśli masz już urządzenia, które są już podłączone do programu [Microsoft Defender for Endpoint](/windows/security/threat-protection/), będą one już wyświetlane na liście urządzeń zarządzanych. Wykonaj [krok 3. Jeśli masz urządzenia podłączone do programu Microsoft Defender for Endpoint](insider-risk-management-settings.md#OnboardStep3) w następnej sekcji.

W tym scenariuszu wdrażania będziesz na urządzeniach, które nie zostały jeszcze podłączone, i chcesz tylko monitorować działania związane z ryzykiem w niejawnym programie testów na Windows 10 urządzeniach.
...
Urządzenia z systemem MacOS (Catalina w wersji 10.15 lub nowszej) można dołączać do programu Microsoft 365 w celu obsługi zasad zarządzania ryzykiem w niejawnym programie testów przy użyciu usługi Intune lub usługi JAMF Pro. Aby uzyskać więcej informacji i wskazówki dotyczące konfiguracji, zobacz [Dołączanie urządzeń z systemem macOS do Microsoft 365 (wersja zapoznawcza)](device-onboarding-macos-overview.md).
...
Ustawienia inteligentnego wykrywania pomagają uściślić sposób przetwarzania wykrywania ryzykownych działań na alerty. W niektórych przypadkach może być konieczne zdefiniowanie typów plików do ignorowania lub wymuszanie poziomu wykrywania dla plików w celu zdefiniowania minimalnego paska alertów. Za pomocą tych ustawień możesz kontrolować ogólną głośność alertów, wykluczenia typów plików i limity liczby plików.
...
### <a name="threshold-for-unusual-file-activity"></a>Próg nietypowej aktywności dotyczącej plików

Aby określić minimalny poziom plików przed raportem alertów aktywności w zasadach ryzyka niejawnego programu testów, wprowadź liczbę plików. Na przykład wpisz "10", jeśli nie chcesz generować alertów o ryzyku niejawnego programu testów, gdy użytkownik pobrał nie więcej niż 10 plików, nawet jeśli zasady uznasz to działanie za nietypowe.
...
| Komentarze | Komentarze domyślne alertu. Wartości to *Nowy alert* (zarejestrowany po utworzeniu alertu) i *Zaktualizowano alert* (rejestrowane w przypadku aktualizacji alertu). Za pomocą funkcji AlertID koreluj aktualizacje alertu. |
...
Powiadomienia administratora automatycznie wysyłają powiadomienia e-mail do użytkowników uwzględnionych w grupach ról Zarządzanie ryzykiem w niejawnym  programie testów *,* Analitycy zarządzających ryzykiem w niejawnym programie testów oraz Grupy ról zarządzania ryzykiem w niejawnym programie testów po wygenerowaniu pierwszego alertu dla nowych zasad. Ta funkcja jest domyślnie włączona dla wszystkich organizacji, a zasady są sprawdzane co 24 godziny dla alertów przy pierwszym czasie. Powiadomienia nie są wysyłane dla alertów, które występują w zasadach po pierwszym alertie.
