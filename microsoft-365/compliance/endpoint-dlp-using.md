...
Te scenariusze wymagają, aby urządzenia były już dołączone i raportowane w Eksploratorze działań. Jeśli urządzenia nie zostały jeszcze dołączone, zobacz [Wprowadzenie z zapobieganiem utracie danych punktu końcowego](endpoint-dlp-getting-started.md).
...
7. Wybierz pozycję **Inspekcja lub ograniczanie działań na urządzeniach Windows** i pozostaw akcje ustawione **tylko na Wartość Inspekcja**. Wybierz przycisk **Dalej**.
...
5. Przewiń w dół do sekcji **Inspekcja lub ograniczanie działań na urządzeniu Windows** i dla każdego działania ustaw odpowiednią akcję na **Wartość Blokuj z zastąpieniem**.
...
W tym scenariuszu synchronizacja plików z etykietą **poufności o wysokim poziomie poufności** z OneDrive jest zablokowana. Jest to złożony scenariusz z wieloma składnikami i procedurami. Potrzebne będą następujące elementy:

- Konto użytkownika AAD docelowe i dołączony komputer Windows 10, który już synchronizuje lokalny folder OneDrive z magazynem OneDrive w chmurze.
- Microsoft Word zainstalowany na komputerze docelowym Windows 10
- Skonfigurowane i opublikowane etykiety poufności — zobacz [Wprowadzenie z etykietami poufności](get-started-with-sensitivity-labels.md#get-started-with-sensitivity-labels) oraz [Tworzenie i konfigurowanie etykiet poufności i ich zasad](create-sensitivity-labels.md#create-and-configure-sensitivity-labels-and-their-policies).
...
3. Utwórz dokument programu Word na urządzeniu Windows 10, do których są przeznaczone zasady, zastosuj etykietę i skopiuj go do folderu OneDrive lokalnego konta użytkownika, który jest synchronizowany.  
...
3. Wybierz pozycję **Dodaj lub edytuj niedozwolone aplikacje** i dodaj *OneDrive* jako nazwę wyświetlaną i nazwę wykonywalną *onedrive.exe*, aby uniemożliwić onedrive.exe uzyskiwanie dostępu do elementów **etykiety Wysoce poufne**.
...
7. Wprowadź ścieżkę do folderu na maszynach lokalnych, do którego mają zostać przeniesione oryginalne pliki poufne. Przykład:
    ...
    *C:\Users\IsaiahLanger\Microsoft DLP\Quarantine\OneDrive*
    ...
    > [!NOTE]
    > Automatyczna kwarantanna DLP spowoduje utworzenie podfolderów dla plików dla każdej niedozwolonej aplikacji. Jeśli więc masz zarówno *Notatnik*, jak i *OneDrive* na liście nieobsługiwanych aplikacji, zostanie utworzony podfolder \**OneDrive** i inny podfolder \**Notatnik**.
...
8. Wybierz **pozycję Zastąp pliki pliki .txt zawierającym następujący tekst** i wprowadź tekst, który chcesz w pliku zastępczym. Na przykład dla pliku o nazwie *auto quar 1.docx*:
    ...
    > auto quar 1.docx zawiera poufne informacje, które twoja organizacja chroni za pomocą zasad ochrony przed utratą danych (DLP) i została przeniesiona do folderu kwarantanny: C:\Users\IsaiahLanger\Microsoft DLP\Quarantine\OneDrive\auto quar 1_20210728_151541.docx.

9. Wybierz **pozycję Zapisz**

### <a name="configure-a-policy-to-block-onedrive-synchronization-of-files-with-the-sensitivity-label-highly-confidential"></a>Konfigurowanie zasad w celu blokowania OneDrive synchronizacji plików z etykietą poufności Wysoce poufne
...
7. Utwórz regułę z następującymi wartościami:
    ...
    1.  **Działania** >  **Inspekcja lub ograniczanie działań na urządzeniach** >  Windows **Uzyskaj dostęp przez niedozwolone aplikacjeBlokuj** > . Na potrzeby tego scenariusza wyczyść wszystkie inne działania.
...
1. Zaloguj się na komputerze Windows 10 przy użyciu konta użytkownika określonego w artykule [Konfigurowanie zasad w celu blokowania OneDrive synchronizacji plików za pomocą etykiety poufności Wysoce poufne](#configure-a-policy-to-block-onedrive-synchronization-of-files-with-the-sensitivity-label-highly-confidential) krok 5.

2. Utwórz folder, którego zawartość nie zostanie zsynchronizowana z OneDrive. Przykład:

    *Folder źródłowy C:\automatycznej kwarantanny*

3. Otwórz Microsoft Word i utwórz plik w folderze źródłowym automatycznej kwarantanny. Zastosuj etykietę **Wysoce poufne** poufności; Zobacz [Stosowanie etykiet poufności do plików i wiadomości e-mail w Office](https://support.microsoft.com/topic/apply-sensitivity-labels-to-your-files-and-email-in-office-2f96e7cd-d5a4-403b-8bd7-4cc636bae0f9).

4. Skopiuj utworzony plik do folderu synchronizacji OneDrive. Powinno zostać wyświetlone wyskakujące powiadomienie użytkownika informujące o tym, że akcja jest niedozwolona i że plik zostanie poddany kwarantannie. Na przykład dla nazwy użytkownika *Isaiah Langer* i dokumentu zatytułowanego *dokument automatycznej kwarantanny 1.docx* zobaczysz następujący komunikat:

    ![Wyskakujące powiadomienie użytkownika dotyczące zapobiegania utracie danych z informacją, że akcja synchronizacji OneDrive nie jest dozwolona dla określonego pliku i że plik zostanie poddany kwarantannie.](../media/auto-quarantine-user-notification-toast.png)
    
    Komunikat brzmi:
    
    > Otwieranie dokumentu autokwarantyny 1.docx z tą aplikacją jest niedozwolone. Plik zostanie poddany kwarantannie "C:\Users\IsaiahLanger\Microsoft DLP\OneDrive"
...
3. Skonfiguruj zasady DLP, aby zdefiniować rodzaje elementów poufnych, dla których przekazywanie powinno być ograniczone do tych miejsc, włączając **Upload usługom w chmurze** i **dostępowi z niedozwolonej przeglądarki**.
