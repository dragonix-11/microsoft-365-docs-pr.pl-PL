...
Jeśli masz tylko subskrypcję usługi Microsoft 365, zobacz Opis rachunku lub faktury za Microsoft 365 [dla firm](understand-your-invoice2.md).

## <a name="understand-the-invoice-header"></a>Opis nagłówka faktury

W górnej części pierwszej strony jest określana, kto jest odpowiedzialna za płatność, gdzie jest wysyłany rachunek, oraz podsumowanie opłat.

| Termin | Opis |
| --- | --- |
| Sprzedano do |Konto rozliczeniowe, które określa nazwę i adres podmiotu prawnego odpowiedzialnego za płatność. Tą informacjami można zarządzać <a href="https://go.microsoft.com/fwlink/p/?linkid=2084771" target="_blank">na stronie</a> Konta rozliczeniowe, gdzie można znaleźć umowę dotycząca konta oraz zarządzać rolami i uprawnieniami. |
| Faktura dla |Określa, kto otrzymuje fakturę. Informacjami tym można zarządzać na stronie <a href="https://go.microsoft.com/fwlink/p/?linkid=2103629" target="_blank">Profile rozliczeniowe</a> . Profil rozliczeniowy jest również widoczny na stronie faktury online w sekcji **Podsumowanie** faktury. Aby dowiedzieć się więcej na temat profilów rozliczeń i sposobu ich używania do tworzenia bardziej elastycznych opcji rozliczeń dla organizacji, zobacz Zarządzanie [profilami rozliczeniowymi](manage-billing-profiles.md). |
| Profil rozliczeniowy |Nazwa profilu rozliczeniowego służącego do definiowania właściwości **faktury, takich** jak Faktura **, Numer** zakupu i warunki płatności. Informacjami tym można zarządzać na stronie <a href="https://go.microsoft.com/fwlink/p/?linkid=2103629" target="_blank">Profile rozliczeniowe</a> . Aby uzyskać więcej informacji na temat profilów rozliczeń i sposobu używania ich do tworzenia bardziej elastycznych opcji rozliczeń dla organizacji, zobacz Zarządzanie [profilami rozliczeń](manage-billing-profiles.md). |
| Numer faktury |Unikatowy numer faktury wygenerowany przez firmę Microsoft, używany do śledzenia. |
| Data faktury |Data wygenerowania faktury (zazwyczaj od 5 do 12 dni po zakończeniu cyklu rozliczeniowego). Możesz sprawdzić datę faktury na stronie szczegółów profilu rozliczeniowego. Opłaty występujące między końcem okresu rozliczeniowego a datą faktury są uwzględnione na fakturze za następny miesiąc, ponieważ znajdują się w następnym okresie rozliczeniowym. Daty rozpoczęcia i zakończenia okresu rozliczeniowego dla każdej faktury są podane w formacie PDF faktury powyżej **podsumowania rozliczeń**.|
| Warunki płatności |Jak płacisz za rachunek Microsoft. *30 dni netto* oznacza, że płatność jest opłacana zgodnie z instrukcjami podanymi na fakturze, w ciągu 30 dni od daty wystawienia faktury. |

## <a name="understand-the-billing-summary"></a>Opis podsumowania rozliczeń

Podsumowanie **rozliczeń zawiera** podsumowanie opłat od poprzedniego okresu rozliczeniowego, wszelkich zastosowanych środków, podatku i całkowitej należnej kwoty.

| Termin | Opis |
| --- | --- |
| Opłaty|Łączna liczba produktów zakupionych w tym okresie rozliczeniowym oraz powiązane z nimi opłaty i podatki. Zakupy są agregowane, aby zapewnić zwięzły widok rachunku. |
| Środki |Środki otrzymane ze zwrotów |
| Środki na korzystanie z systemu Azure |Twoje środki na korzystanie z systemu Azure, które są automatycznie stosowane do opłat za korzystanie z systemu Azure w każdym okresie rozliczeniowym. Jeśli nie masz żadnych środków na korzystanie z systemu Azure, to pole jest ukryte. Aby uzyskać więcej informacji na temat środków na korzystanie z systemu Azure, zobacz Śledzenie salda środków na korzystanie z usługi [Azure w umowie klienta firmy Microsoft](/azure/billing/billing-mca-check-azure-credits-balance). |
| Suma częściowa |Kwota przed opodatkowaniem |
| Podatek |Typ i kwota podatku do zapłacenia w zależności od kraju profilu rozliczeniowego. Jeśli nie musisz płacić podatku, na fakturze nie będzie widoczny żaden podatek. |

### <a name="understand-your-charges"></a>Opis opłat

Na stronach opłat jest pokazywany koszt rozbity na produkty. W przypadku klientów korzystających z systemu Azure opłaty mogą być uporządkowane według sekcji faktury. Aby uzyskać więcej informacji na temat sposobu, w jaki sekcje faktur są [](/azure/billing/billing-mca-overview#invoice-sections) używane z produktami platformy Azure, zobacz sekcje Faktur w sekcji Wprowadzenie do konta rozliczeniowego [Umowy klienta firmy Microsoft](/azure/billing/billing-mca-overview). W ramach każdego zamówienia produktu koszt jest podzielone według rodziny usług.

| Termin |Opis |
| --- | --- |
| Cena jednostkowa | Efektywna cena jednostkowa usługi (w walucie cennika), która jest używana do obliczania opłaty. Ta cena jest unikatowa w przypadku produktu, rodziny usługi, licznika i oferty. |
| Ilość | Liczba zakupionych lub zużywanych w okresie rozliczeniowym |
| Opłaty/środki | Kwota netto opłat po zastosowaniu środków/zwrotów kosztów |
| Środków na korzystanie z systemu Azure | Kwota środków na korzystanie z systemu Azure zastosowanych do opłat/środków |
| Stawka podatku | Stawka podatku w zależności od kraju |
| Kwota podatku | Kwota podatku zastosowana do zakupu na podstawie stawki podatkowej |
| Suma | Całkowita kwota należna za zakup |

Szczegóły pozycji różnią się w zależności od typu produktu, za który jest naliczana opłata. Na przykład w przypadku produktów Azure wyświetlana jest kwota zastosowanych środków na korzystanie z systemu Azure. W przypadku produktów opartych na stanowiskoch jest pokazywana cena jednostkowa i ilość. W szczegółach faktury są wyświetlane zakupione produkty, rabaty lub środki, stawka podatku i kwota oraz sumy pozycji.
...
Jeśli masz pytania lub potrzebujesz pomocy dotyczącej środków na korzystanie z systemu Azure, utwórz <a href="https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest" target="_blank">wniosek o pomoc techniczną, aby uzyskać pomoc techniczną dla systemu Azure</a>.
