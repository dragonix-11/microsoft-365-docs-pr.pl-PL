---
title: Wdrożenie oparte na aplikacji dla Ochrona punktu końcowego w usłudze Microsoft Defender na iOS
ms.reviewer: ''
description: Opis sposobu wdrażania Ochrona punktu końcowego w usłudze Microsoft Defender na iOS przy użyciu aplikacji
...
ms.locfileid: "65669975"
---
# <a name="deploy-microsoft-defender-for-endpoint-on-ios"></a>Wdrażanie Ochrona punktu końcowego w usłudze Microsoft Defender na iOS
...
W tym temacie opisano wdrażanie usługi Defender for Endpoint na iOS na Intune — Portal firmy zarejestrowanych urządzeniach. Aby uzyskać więcej informacji na temat rejestracji urządzeń Intune, zobacz [Rejestrowanie urządzeń z systemem iOS/iPadOS w Intune](/mem/intune/enrollment/ios-enroll).
...
- Upewnij się, iOS rejestracja jest wykonywana dla użytkowników. Użytkownicy muszą mieć przypisaną licencję usługi Defender for Endpoint, aby korzystać z usługi Defender for Endpoint na iOS. Aby uzyskać instrukcje dotyczące przypisywania [licencji, zobacz Przypisywanie licencji użytkownikom](/azure/active-directory/users-groups-roles/licensing-groups-assign) .

> [!NOTE]
> Ochrona punktu końcowego w usłudze Microsoft Defender na iOS jest dostępna w [App Store Firmy Apple](https://aka.ms/mdatpiosappstore).

## <a name="deployment-steps"></a>Kroki wdrażania

Wdróż usługę Defender for Endpoint na iOS za pośrednictwem Intune — Portal firmy.

### <a name="add-ios-store-app"></a>Dodawanie aplikacji ze sklepu iOS

1. W [centrum administracyjnym programu Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) przejdź do pozycji **Aplikacje** ->  **iOS/iPadOS** -> **Dodaj** ->  **iOS aplikacji ze sklepu**, a następnie kliknij pozycję **Wybierz**.

    > [!div class="mx-imgBorder"]
    > :::image type="content" source="images/ios-deploy-1.png" alt-text="Karta Dodawanie aplikacji w centrum Microsoft Endpoint Manager Administracja" lightbox="images/ios-deploy-1.png":::
...
1. Wybierz **pozycję iOS 11.0** jako minimalny system operacyjny. Przejrzyj pozostałe informacje o aplikacji i kliknij przycisk **Dalej**.

1. W sekcji **Przypisania** przejdź do sekcji **Wymagane** i wybierz pozycję **Dodaj grupę**. Następnie możesz wybrać grupy użytkowników, które chcesz skierować do usługi Defender for Endpoint w aplikacji iOS. Kliknij **pozycję Wybierz,** a następnie **pozycję Dalej**.

    > [!NOTE]
    > Wybrana grupa użytkowników powinna składać się z Intune zarejestrowanych użytkowników.

    > [!div class="mx-imgBorder"]
    > :::image type="content" source="images/ios-deploy-2.png" alt-text="Karta Dodaj grupę w centrum Microsoft Endpoint Manager Administracja" lightbox="images/ios-deploy-2.png":::
...
Następnie skonfiguruj tryb nadzorowany dla aplikacji Defender for Endpoint za pomocą zasad App Configuration.
   ...
   > [!NOTE]
   > Te zasady konfiguracji aplikacji dla urządzeń nadzorowanych mają zastosowanie tylko do urządzeń zarządzanych i powinny być przeznaczone dla wszystkich zarządzanych urządzeń iOS jako najlepsze rozwiązanie.
...
1. Zaloguj się do [centrum administracyjnego Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) i przejdź do obszaru **Zasady** \> konfiguracji aplikacji **Aplikacje** \> **Dodaj**. Wybierz pozycję **Urządzenia zarządzane**.
    ...
    > [!div class="mx-imgBorder"]
    > ![Obraz przedstawiający Microsoft Endpoint Manager Administracja Center4.](images/ios-deploy-4.png)
...
1. Na stronie *Tworzenie zasad konfiguracji aplikacji* podaj następujące informacje:
    ...
    > [!div class="mx-imgBorder"]
    > ![Obraz przedstawiający Microsoft Endpoint Manager Administracja Center5.](images/ios-deploy-5.png)
...
1. Na następnym ekranie wybierz pozycję **Użyj projektanta konfiguracji** jako formatu. Określ następującą właściwość:
    ...
    > [!div class="mx-imgBorder"]
    > ![Obraz przedstawiający Microsoft Endpoint Manager Administracja Center6.](images/ios-deploy-6.png)
...
1. Następnie należy wdrożyć profil niestandardowy na nadzorowanych urządzeniach iOS. Dotyczy to rozszerzonych możliwości ochrony przed wyłudzaniem informacji. Wykonaj poniższe kroki:

    - Pobierz profil konfiguracji z [https://aka.ms/mdeiosprofilesupervised](https://aka.ms/mdeiosprofilesupervised)
    - Przejdź do **pozycji Urządzenia** ->  iOS/**profile** ->  konfiguracji **systemu iPadOS** -> **Utwórz profil**

    > [!div class="mx-imgBorder"]
    > ![Obraz przedstawiający Microsoft Endpoint Manager Administracja Center7.](images/ios-deploy-7.png)
    ...
    - W sekcji **Przypisania** wybierz grupę urządzeń, do której chcesz zastosować ten profil. Najlepszym rozwiązaniem jest zastosowanie tego rozwiązania do wszystkich zarządzanych urządzeń iOS. Wybierz pozycję **Dalej**.
...
1. W [centrum administracyjnym programu Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) przejdź do pozycji **Profile** ->  konfiguracji **urządzeń** -> **Utwórz profil**.
1. Wybierz **pozycję Platforma** jako **typ iOS/iPadOS** i **Profil** jako **sieć VPN**. Kliknij **pozycję Utwórz**.
...
1. Wybierz pozycję **Niestandardowa sieć VPN** dla pozycji Typ połączenia i w sekcji **Podstawowa sieć VPN** wprowadź następujące elementy:
    ...
    - Wybierz pozycję **Dodaj** dla **reguł na żądanie** i wybierz **pozycję Chcę wykonać następujące czynności = Połączenie sieci VPN**, **do których chcesz ograniczyć wartość = Wszystkie domeny**.
...
Po zakończeniu i zsynchronizowania powyższej konfiguracji z urządzeniem na docelowych urządzeniach iOS są wykonywane następujące akcje:
...
1. Po zainstalowaniu usługi Defender for Endpoint na iOS na urządzeniu zostanie wyświetlona ikona aplikacji.
...
2. Naciśnij ikonę aplikacji Defender for Endpoint (MSDefender) i postępuj zgodnie z instrukcjami wyświetlanymi na ekranie, aby wykonać kroki dołączania. Szczegóły obejmują akceptację przez użytkownika końcowego uprawnień iOS wymaganych przez usługę Defender for Endpoint w iOS.
...
Intune umożliwia skonfigurowanie aplikacji Defender for iOS za pomocą zasad App Configuration.
...
- [Konfigurowanie usługi Defender dla punktu końcowego w funkcjach iOS](ios-configure-features.md)
