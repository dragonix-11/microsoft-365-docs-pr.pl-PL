---
title: Interfejs API akcji listy
description: Dowiedz się, jak korzystać z interfejsu API List MachineActions w celu pobrania kolekcji akcji komputera w programie Microsoft Defender for Endpoint.
keywords: apis, api Graph, obsługiwane api, kolekcja akcji maszynowych
...
Pobiera kolekcję [akcji komputera](machineaction.md).

Obsługuje [zapytania OData w wersji 4](https://www.odata.org/documentation/).

Zapytanie OData jest obsługiwane `$filter` dla: `id`, `status`, `machineId`, `type`, `requestor`i `creationDateTimeUtc` właściwości.
...
Jeśli ta metoda się powiedzie, ta metoda zwróci wartość 200, kod odpowiedzi OK z kolekcją obiektów [akcji](machineaction.md) maszynowej.

## <a name="example-1"></a>Przykład 1

### <a name="example-1-request"></a>Przykład 1 żądania

Oto przykład żądania w organizacji, która ma trzy działania maszynowe.
...
Oto przykład żądania filtrowania akcji według identyfikatora komputera i dwóch najnowszych akcji.
