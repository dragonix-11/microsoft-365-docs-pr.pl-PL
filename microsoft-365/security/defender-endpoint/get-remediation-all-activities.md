...
Zwraca informacje o wszystkich działaniach korygowania.

[Dowiedz się więcej o działaniach korygowania](tvm-remediation.md).

**ADRES URL:** GET: /api/remediationTasks
<br>Obsługuje [zapytania OData V4](https://www.odata.org/documentation/).
<br>Obsługiwane operatory OData:
<br>```$filter``` on:  ```createdon``` i ```status``` właściwości.
<br>```$top``` z maksymalną wartością 10 000.
<br>```$skip```.
<br>Zobacz przykłady w [temacie Zapytania OData z Ochrona punktu końcowego w usłudze Microsoft Defender](exposed-apis-odata-samples.md).
...
Delegowane (konto służbowe)|RemediationTask.Read|\'Przeczytaj informacje o lukach w zabezpieczeniach dotyczące zarządzania zagrożeniami i lukami w zabezpieczeniach\'
