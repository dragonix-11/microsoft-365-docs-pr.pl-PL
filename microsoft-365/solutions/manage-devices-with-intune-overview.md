...
|2|Rejestrowanie urządzeń do Intune|To zadanie wymaga więcej czasu i planowania do zaimplementowania. Firma Microsoft zaleca używanie Intune do rejestrowania urządzeń, ponieważ to narzędzie zapewnia optymalną integrację. Istnieje kilka opcji rejestrowania urządzeń, w zależności od platformy. Na przykład urządzenia Windows można zarejestrować przy użyciu funkcji Dołączanie do usługi Azure AD lub przy użyciu rozwiązania Autopilot. Musisz przejrzeć opcje dla każdej platformy i zdecydować, która opcja rejestracji jest najlepsza dla Twojego środowiska. Aby uzyskać więcej informacji, zobacz [Krok 3— Rejestrowanie urządzeń do Intune](manage-devices-with-intune-enroll.md).|E3, E5, F1, F3, F5|
