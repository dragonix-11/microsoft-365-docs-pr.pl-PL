...
Nie pomijaj tej sekcji. Często uważam, że muszę cofnąć się do tych tematów, nawet w przypadku klientów, którzy od lat korzystają z usług w chmurze.
...
- Poziom 4 to miejsce, w którym znajdują się rzeczywiste dane. Ta "płaszczyzna danych" jest złożonym tematem. Niektóre usługi korzystają z usługi Azure AD na potrzeby kontroli dostępu opartej na rolach, inne nie. Omówię to nieco, gdy przejdziemy do tematów delegowania.
...
Kilka razy wspomniałem o [usłudze Azure AD PIM](/azure/active-directory/privileged-identity-management/) . Istnieje odpowiednie rozwiązanie do zarządzania [dostępem](/microsoft-identity-manager/pam/privileged-identity-management-for-active-directory-domain-services) uprzywilejowanym (PAM) Microsoft Identity Manager (MIM) dla kontrolek lokalnych. Warto również zapoznać się ze [stacjami roboczymi z dostępem uprzywilejowanym](/windows-server/identity/securing-privileged-access/privileged-access-workstations) (PAW) i [zarządzaniem tożsamościami usługi Azure AD](/azure/active-directory/governance/identity-governance-overview). Istnieją również różne narzędzia innych firm, które mogą włączyć just in time, just-enough i dynamiczne podniesienie roli. Zazwyczaj jest to część szerszej dyskusji na temat zabezpieczania środowiska.

Czasami scenariusze wymagają dodania użytkownika zewnętrznego do roli (zobacz sekcję z wieloma dzierżawami powyżej). To działa dobrze. [Usługa Azure AD B2B](/azure/active-directory/b2b/) to kolejny duży i zabawny temat, przez który klienci mogą przejść, być może w innym artykule.
